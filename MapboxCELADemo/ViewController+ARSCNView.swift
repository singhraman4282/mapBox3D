...
extension ViewController: ARSCNViewDelegate, ARSessionDelegate {
    ...
    func renderer(_ renderer: SCNSceneRenderer, willRenderScene scene: SCNScene, atTime time: TimeInterval) {
        DispatchQueue.main.async {
            if self.xPosOutlet.isHighlighted {
                self.arView?.scene.rootNode.enumerateChildNodes({ (node, _) in
                    if node.name == "endSphere" {
                        let moveAction = SCNAction.move(by: SCNVector3(self.moveFactor,0,0), duration: 0.1)
                        node.runAction(moveAction)
                    }
                })
            }
            if self.yPosOutlet.isHighlighted {
                self.arView?.scene.rootNode.enumerateChildNodes({ (node, _) in
                    if node.name == "endSphere" {
                        let moveAction = SCNAction.move(by: SCNVector3(0,self.moveFactor,0), duration: 0.1)
                        node.runAction(moveAction)
                    }
            ...
            if self.zPosOutlet.isHighlighted {
                self.arView?.scene.rootNode.enumerateChildNodes({ (node, _) in
                    if node.name == "endSphere" {
                        let moveAction = SCNAction.move(by: SCNVector3(0,0,self.moveFactor), duration: 0.1)
                        node.runAction(moveAction)
                    }
                })
            }
            if self.xNegOutlet.isHighlighted {
                self.arView?.scene.rootNode.enumerateChildNodes({ (node, _) in
                    if node.name == "endSphere" {
                        let moveAction = SCNAction.move(by: SCNVector3(-self.moveFactor,0,0), duration: 0.1)
                        node.runAction(moveAction)
            ...
            if self.yNegOutlet.isHighlighted {
                self.arView?.scene.rootNode.enumerateChildNodes({ (node, _) in
                    if node.name == "endSphere" {
                        let moveAction = SCNAction.move(by: SCNVector3(0,-self.moveFactor,0), duration: 0.1)
                        node.runAction(moveAction)
            ...
            if self.zNegOutlet.isHighlighted {
                self.arView?.scene.rootNode.enumerateChildNodes({ (node, _) in
                    if node.name == "endSphere" {
                        let moveAction = SCNAction.move(by: SCNVector3(0,0,-self.moveFactor), duration: 0.1)
                        node.runAction(moveAction)
